...
class StandardWeatherModel(models.Model):
    dt = models.BigIntegerField(
        primary_key=True, unique=True, help_text="Current time, Unix, UTC"
    )
    pressure = models.IntegerField(
        help_text="Atmospheric pressure on the sea level. Units: hPa"
    )
    ...
    wind_deg = models.FloatField(
        help_text="Wind direction. Units: degrees(meteorological)"
    )
    ...
    weather_main = models.CharField(
        max_length=20, help_text=" Group of weather parameters (Rain, Snow etc.)"
    )
    weather_description = models.CharField(
        max_length=50, help_text="Weather condition within the group "
    )
    weather_icon = models.CharField(
        max_length=3, help_text="Weather Icon code. E.g 04d"
    )
...
class CurrentWeather(StandardWeatherModel):
    ...
    visibility = models.FloatField(
        help_text=" Average visibility. Units: m, Max value is 10 km"
    )
...
class DailyWeather(StandardWeatherModel):
    ...
    moonrise = models.DateTimeField(
        help_text="The time of when the moon rises for this day, Unix, UTC"
    )
    moonset = models.DateTimeField(
        help_text="The time of when the moon sets for this day, Unix, UTC"
    )
    summary = models.TextField(
        help_text="Human-readable description of the weather conditions for the day"
    )
...
class Alert(models.Model):
    ...
    start = models.DateTimeField(
        help_text="Date and time of the start of the alert, Unix, UTC"
    )
    end = models.DateTimeField(
        help_text="Date and time of the end of the alert, Unix, UTC"
    )
